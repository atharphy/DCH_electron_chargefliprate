...
string pairFunc_dilep(int m, int n, string cat, double Zwindow) {
    TLorentzVector lep1 = LepV(m);
    TLorentzVector lep2 = LepV(n);

    int c1 = (m==1) ? q_1 : (m==2) ? q_2 : (m==3) ? q_3 : q_4;
    int c2 = (n==1) ? q_1 : (n==2) ? q_2 : (n==3) ? q_3 : q_4;

    double mass = (lep1 + lep2).M();
    double mZ = 91.2;
    double diff = fabs(mass - mZ);

    bool isSameFlavor = (cat[m-1] == cat[n-1]) && (cat[m-1] == 'e' || cat[m-1] == 'm');
    bool isOS = (c1 * c2 < 0);
    bool isSS = (c1 * c2 > 0);

    if (isSameFlavor) {
        if (diff < Zwindow) {
            if (isOS) return "Zwindow_OS";
            else if (isSS) return "Zwindow_SS";
        } else {
            if (isOS) return "Zveto_OS";
            else if (isSS) return "Zveto_SS";
        }
    } else {
        if (isOS) return "oppPair";
        else if (isSS) return "sameSign";
    }
